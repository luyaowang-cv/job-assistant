...
const form = reactive({ type: 'PROJECT', title: '', organization: '', role: '', techStackText: '', tags: [] as string[], startDate: '', endDate: '', current: false, projectUrl: '', variantName: '标准版', content: '' })
const variant = reactive({ name: '', content: '' })
const impact = ref<{ previewToken: string; targets: Array<{ id: string; title: string; reason: string }> } | null>(null)
const selectedTargets = ref<string[]>([])
const timedTypes = new Set(['PROJECT', 'INTERNSHIP', 'WORK', 'CAMPUS', 'AWARD'])
const detailedTypes = new Set(['PROJECT', 'INTERNSHIP', 'WORK', 'CAMPUS', 'AWARD'])
...
const titleLabels: Record<string, string> = { PROJECT: '项目名称', INTERNSHIP: '公司名称', WORK: '公司名称', CAMPUS: '组织或活动名称', AWARD: '奖项名称' }
const titlePlaceholders: Record<string, string> = { PROJECT: '例如：多 Agent 前端迁移系统', INTERNSHIP: '例如：搜狐', WORK: '例如：某科技公司', CAMPUS: '例如：校学生会', AWARD: '例如：全国大学生竞赛一等奖' }
const roleLabels: Record<string, string> = { PROJECT: '项目职责 / 角色', INTERNSHIP: '实习职位', WORK: '工作职位', CAMPUS: '担任角色', AWARD: '奖项等级' }
const organizationLabels: Record<string, string> = { PROJECT: '所属组织 / 公司', INTERNSHIP: '部门 / 团队（可选）', WORK: '部门 / 团队（可选）', CAMPUS: '学校 / 社团（可选）', AWARD: '颁发机构（可选）' }
...
function beginCreate() {
  ...
  Object.assign(form, { type: 'PROJECT', title: '', organization: '', role: '', techStackText: '', tags: [], startDate: '', endDate: '', current: false, projectUrl: '', variantName: '标准版', content: '' })
...
async function createCard() {
  ...
  try {
    ...
    if (editingId.value) {
      await $fetch(`/api/v1/material-cards/${editingId.value}`, { method: 'PATCH', body: { title: form.title, tags: form.tags, facts } })
    }
    else {
      await $fetch('/api/v1/material-cards', { method: 'POST', body: { type: form.type, title: form.title, tags: form.tags, facts, variant: { name: form.variantName, content: form.content } } })
    }
    editorOpen.value = false
    editingId.value = null
    await load()
    ElMessage.success(wasEditing ? '素材卡片已更新。' : '素材卡片已创建。')
  } catch { ElMessage.error('创建失败，请检查标题和文案。') } finally { saving.value = false }
}
async function addVariant() {
  ...
  try {
    ...
    Object.assign(variant, { name: '', content: '' })
    await load()
  ...
  <section class="page-rail material-page">
      ...
      <div v-loading="loading" class="material-layout">
        <div class="card-list" role="list" aria-label="素材卡片">
          <button v-for="card in cards" :key="card.id" class="material-card-row" :class="{ active: selected?.id === card.id }" type="button" @click="selected = card">
            <span class="material-card-row__type">{{ typeLabels[card.type] }}</span><strong>{{ card.title }}</strong><el-tag v-if="card.archivedAt" type="info">已归档</el-tag>
    ...
    <el-dialog v-model="editorOpen" :title="editingId ? '编辑素材卡片' : '新建素材卡片'" width="min(720px, 92vw)">
      <el-form label-position="top">
        ...
        <div class="dialog-grid">
          <el-form-item v-if="!editingId" label="版本名称"><el-input v-model="form.variantName" placeholder="例如：标准版" /></el-form-item>
          <div v-if="!editingId" class="facts-note"><b>结构化信息由系统保存</b><span>角色、组织、时间和技术栈会自动进入固定模板，不需要填写 JSON。</span></div>
        </div>
