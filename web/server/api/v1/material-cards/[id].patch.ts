...
import { getMaterialCard, updateMaterialCard } from '../../../services/material-card.service'
...
export default defineEventHandler(async (event) => {
  ...
  const existing = await getMaterialCard(id.data.id, false)
  if (!existing) return apiError(event, 404, 'MATERIAL_CARD_NOT_FOUND', 'Active material card not found.')
  if (body.data.title === '' && existing.type !== 'SELF_EVALUATION') {
    return validationError(event, [{ code: 'custom', path: ['title'], message: 'Title is required for this material type.' }])
  }
