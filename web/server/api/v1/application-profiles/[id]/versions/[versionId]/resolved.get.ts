...
import { resolveApplicationProfileVersion } from '../../../../../../services/document-composition.service'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await resolveApplicationProfileVersion(route.data.id, route.data.versionId)
  return result ? apiSuccess(event, result) : apiError(event, 404, 'APPLICATION_PROFILE_VERSION_NOT_FOUND', 'Application profile version not found.')
})
